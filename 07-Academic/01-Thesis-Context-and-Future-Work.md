...
| **JWT RS256 + JWKS** | stateless verification at every service without sharing the private key; industry-standard. Auth publishes its public key at `/internal/v1/.well-known/jwks.json` and **all six other services** resolve keys from it by `kid`, so rotating the pair is a change at Auth alone — no redeploy anywhere else. Publishing the previous key alongside the new one during an overlap means rotation also does not force users to log in again: tokens already issued keep validating until they expire. Only Auth holds signing material. | three separate client implementations were needed, because the services do not share a JWT stack (`shared-jwt` for the monorepo modules, Nimbus `withJwkSetUri` for Notification/Social, a jjwt `Locator` for Therapist). That duplication is the cost of the repo split, and it is the main argument for consolidating on one JWT library. A JWKS consumer also depends on Auth being reachable *once*; each caches the key set and keeps serving it through an Auth outage, but a consumer restarted during one cannot validate until the first fetch lands. |
