...
| **JWT RS256 + JWKS** | stateless verification at every service without sharing the private key; industry-standard. Auth publishes its public key at `/internal/v1/.well-known/jwks.json` and Dashboard resolves keys from it by `kid`, so rotation does not force a redeploy there. Publishing the previous key alongside the new one during an overlap means rotation also does not force users to log in again — tokens already issued keep validating until they expire. | five of the seven services still take the public key as an env var, so for them rotation remains a redeploy. Migrating each is a one-line compose change; keeping one consumer on JWKS first limits the blast radius of an Auth-availability problem. |
