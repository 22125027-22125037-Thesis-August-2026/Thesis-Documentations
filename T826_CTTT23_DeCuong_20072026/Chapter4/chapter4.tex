...
The home screen is a column of ``today'' cards to be filled in. Figure~\ref{fig:j1-daily-loop} follows the journaling card, and Figure~\ref{fig:j1b-capture} shows sleep, nutrition, steps, relaxation sessions cards. Each card costs a tap or two, and steps walked counted in the background.

\begin{figure}[htbp]
    \centering
    \includegraphics[height=\journeyfigheight]{Chapter4/images/j1b-capture.png}
    \caption{Sleep, nutrition, steps, relaxation sessions.}
    \label{fig:j1b-capture}
\end{figure}

Three things keep the habit from depending on memory, and Figure~\ref{fig:j1d-habit} is all three: a streak per category, a trophy for closing the day's four, and Focus Mode.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{Chapter4/images/j1d-habit.png}
    \caption{What brings the user back}
    \label{fig:j1d-habit}
\end{figure}
...
The same cards behave differently when what they record is bad news, and this is the design decision that most distinguishes the product. A negative check-in routes the user, by the emotion recorded, to one of the cards above (FR5). Anger goes to the breathing card, sadness to the Treasure Box, and the SOS card is one tap from either \ref{fig:j2-low-moment}.
