...
Table~\ref{tab:events} catalogs the domain events exchanged over the broker; each queue is paired with a dead-letter queue, so a message that repeatedly fails processing is parked for inspection rather than blocking the consumer.
