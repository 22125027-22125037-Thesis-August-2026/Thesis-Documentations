...
appointments, availability, a patient roster and profile, clinical notes, the
video session, and their side of the chat in Section~\ref{sec:social}. 

A short intake form (reasons for seeking support, prior counseling,
communication style, an optional LGBTQ+ priority) drives matching, which ranks
...
the start time: a sweep every minute auto-rejects stragglers, and the accept
and reject handlers re-check that deadline so a click landing inside that
minute is still refused. Confirmed appointments freeze an hour before the
...
review has arrived. Ownership checks tie every endpoint to the assigned
therapist, but an assignment buys only this professional record; tracked data
still needs a grant (Section~\ref{sec:datasec}), and the dashboard keeps the
patient's diary, food, sleep, and mood tabs locked until one exists.

Consultations run on Jitsi Meet~\cite{Jitsi}. The dashboard opens the room
beside the patient's stated reason, their permitted diary entries, and a
scratchpad that feeds the note; the mobile app embeds the same room in a
\texttt{WebView} whose injected scripts pin the interface to Vietnamese, hide
the Google moderator sign-in, replace the lobby with a Vietnamese waiting
screen, and confine navigation to the room so hanging up returns the user to
the app. Both clients name the room after the appointment's UUID, unguessable
